...
<head>
  ...
  function oldEvt(apptdate, eventid) {
    ...
    dlgopen('../../main/calendar/add_edit_event.php?date=' + apptdate + '&eid=' + eventid, '_blank', 725, 500, '',
      title);
  }
...
?>
    ...
    top.restoreSession();
    $("#primary_support_ps_expand").load("primary_support_fragment.php", {
        'embeddedScreen': true
    }, function() {})
  ...
  <table class="table_header">
    <tr>
      ...
      <td style='padding-left:1em;' class="delete">
        <a class='css_button deleter' href='../deleter.php?patient=<?php echo htmlspecialchars($pid, ENT_QUOTES); ?>'
...
if ($row['subscriber_country'] != "") {
                ...
                <br><span class='bold'><?php echo htmlspecialchars(xl('Subscriber Employer'), ENT_NOQUOTES); ?>:
                </span><br>
...
// Show Primary Support setMyPatient
if (true) {
    $widgetTitle = xl("Primary Support");
    $widgetLabel = "primary_support";
    $widgetButtonLabel = xl("Edit");
    $widgetButtonLink = "../primary_support/primary_support.php?patient_id=" . $pid;
...
if (isset($pid) && !$GLOBALS['disable_calendar'] && $GLOBALS['appt_recurrences_widget'] && acl_check('patients', 'appt')) {
...
if (isset($pid) && !$GLOBALS['disable_calendar'] && $showpast > 0 && acl_check('patients', 'appt')) {
